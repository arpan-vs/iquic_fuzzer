...
class QUIC : 
    ...
    def send_initial_chlo(self, only_reset):
        ...
        sever_handshak_secret = secret_all.handshake_secret(cipher_suite = 0x1302,shared_key = shared_key, lable = b"s hs traffic")
    ...
    #handshake ACK packet
        ...
        padding = "00" * (1101)
        handshake_clinet_ACK = bytes.fromhex(ACK_frame + padding)
        handshake_header = bytes.fromhex(handshake_header)
        clinet_handshak_secret = secret_all.handshake_secret(cipher_suite = 0x1302,shared_key = SessionInstance.get_instance().shared_key, lable = b"c hs traffic")
    ...
    #send data from client to server 
        ...
        # self.UDPClientSocket.sendto((initial_clinet_data ), ("127.0.0.1", DPORT))
